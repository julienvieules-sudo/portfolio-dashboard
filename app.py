...
# --- EXPOSITION DEVISE ---
st.divider()
st.subheader("Exposition devise")
expo = bilan_clean.groupby('devise')[col_valeur].sum().reset_index()
expo.columns = ['Devise', 'Valeur']
expo['Part'] = (expo['Valeur'] / expo['Valeur'].sum() * 100).round(1)
fig3 = px.pie(expo, values='Valeur', names='Devise',
              title='Répartition USD / EUR',
              color='Devise',
              color_discrete_map={'USD': '#378ADD', 'EUR': '#1D9E75'})
